...
public class ClockPanel extends JPanel {
	...
	// private static final Image BACK_IMG = new
	// ImageIcon("images/back"+(System.currentTimeMillis() %
	// 4)+".jpg").getImage();
	// ⅠⅡⅢⅣⅤⅥⅦⅨⅩⅪⅫ
	private static final String[] TIMES = {"Ⅻ", "Ⅰ", "Ⅱ", "Ⅲ", "Ⅳ", "Ⅴ", "Ⅵ", "Ⅶ",
			"Ⅷ", "Ⅸ", "Ⅹ", "Ⅺ"};
		...
		new Thread() {
			public void run() {
				...
				for (int i = 0; i < backs.length; i++) {
					backImages[i] = new ImageIcon(backs[i].getAbsolutePath())
							.getImage();
				}
				for (int i = 0;; i++) {
	...
	private BufferedImage getImage() {
		...
		g2d.setStroke(new BasicStroke(radius >> 6, BasicStroke.CAP_SQUARE,
		...
		for (int x = 0; x < 60; x++) {
			if (x % 5 == 0) {
				...
				g2d.setStroke(new BasicStroke(radius >> 6,
						BasicStroke.CAP_SQUARE, BasicStroke.JOIN_ROUND)); // 设置新的画刷
				g2d.draw(new Line2D.Float(centerX, centerY - radius, centerX,
						centerY - radius + (radius >> 4)));// 画线
				g2d.drawString("" + TIMES[(x / 5)], centerX - (radius >> 4),
						centerY - radius + (radius / 5));
		...
		g2d.fillRoundRect(centerX - (radius >> 6), centerY - (radius >> 1),
				radius >> 5, (radius >> 4) + (radius >> 1), arc, arc);
		g2d.rotate(-rotates[0], width >> 1, height >> 1);

		// 画分针
		g2d.setColor(new Color(0x228B22));
		g2d.rotate(rotates[1], centerX, centerY);
		g2d.fillRoundRect(centerX - (radius >> 7), centerY
				- ((radius * 3) >> 2), radius >> 6, (radius >> 4)
				+ ((radius * 3) >> 2), arc, arc);
		g2d.rotate(-rotates[1], centerX, centerY);
		...
		g2d.fillRoundRect(centerX - (radius >> 8), centerY
				- ((radius * 7) >> 3), radius >> 7, (radius >> 3)
				+ ((radius * 7) >> 3), arc >> 2, arc >> 2);
		g2d.rotate(-rotates[2], centerX, centerY);
	 ...
	 * 获得指针旋转的角度
	 * 
	 * @return <li>[0] 时针旋转角度</li> <li>[1] 分针旋转角度</li> <li>[2] 秒针旋转角度</li>
	 */
	...
	public void paint(Graphics g) {
		try {
			super.paint(g);
			g.drawImage(backImage, 0, 0, getWidth(), getHeight(), 0, 0,
